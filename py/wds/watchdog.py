...
parser.add_argument('-c','--cw', help='use this option if you have CW (color wrapper)', action='store_true')
parser.add_argument('-f','--cfg', help='use this option if you want to use some cfgFile ',
...
class AppController:
    ...
    def __init__(self,cfgFile=None,colorWrapper=False):
        ...
        if(cfgFile):
            ...
            if(self.flagCW == 'yes'):
                self.cmd = "ps %s axo pid,stat,command |egrep -e '%s$'" % ('+nc',self.prgName)
            else:
                self.cmd = "ps axo pid,stat,command |egrep -e '%s$'" % (self.prgName)
        else:
           #default:
           if(colorWrapper): #default: with CW and watch somebot.py
               self.cmd = "ps +nc axo pid,stat,command |egrep -e 'somebot.py$'"
           else: #default: there is no CW and watch somebot.py
    ...
    def checkSatuse(self):
        try:
            p = subprocess.Popen(self.cmd, shell=True, stdout=self.pipe, stderr=self.pipe, universal_newlines=True)
            ret = p.stdout.read()
            err = p.stderr.read()
            if(err): raise subprocess.SubprocessError(err)
            #print('ERR =>', err)
            print('=>',ret.split())
            if(ret):
                tmp_str = ret.split()
            ...
            print('Error: wrong option for ps try with or without option -c :', er)
            sys.exit(1)
    ...
    def watchApp(self):
        ...
        if(status):
            ...
            try:
                p = subprocess.Popen(self.prgName, stderr=self.pipe, shell=True, start_new_session=True, universal_newlines=True)
                err = p.stderr.read()
                if(err): raise subprocess.SubprocessError(err)
            except subprocess.SubprocessError as er:
                print('Error: %s' % er)
